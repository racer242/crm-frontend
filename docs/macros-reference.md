...
**Макросы диапазонов дат** (вычисляются от локального времени браузера; без суффикса возвращают ISO-строку UTC — абсолютные мгновения, корректные для любой таймзоны сервера; поддерживают форматирование как `{$now.FORMAT}`):

| Макрос                  | Значение                                    |
| ----------------------- | ------------------------------------------- |
| `{$todayStart}`         | Сегодня `00:00:00.000` (локальное)          |
| `{$todayEnd}`           | Сегодня `23:59:59.999` (локальное)          |
| `{$currentWeekStart}`   | Понедельник текущей недели `00:00:00.000`   |
| `{$currentWeekEnd}`     | Воскресенье текущей недели `23:59:59.999`   |

Форматирование (суффикс — строка формата, как у `{$now.FORMAT}`):

| Макрос                              | Пример результата        |
| ----------------------------------- | ------------------------ |
| `{$todayStart.iso}` (или без него)  | `2025-05-08T00:00:00.000Z` |
| `{$todayEnd.timestamp}`             | `1715174399999`          |
| `{$todayStart.DD.MM.YYYY HH:mm:ss}` | `08.05.2025 00:00:00`    |
| `{$currentWeekStart.DD.MM.YYYY}`    | `05.05.2025`             |

Неделя начинается с понедельника. Хелперы: `src/utils/datetime.ts`. Пример применения — быстрые диапазоны панели «Параметры» страницы «Статистика» (`config/pages/promo-instance/stats.json`).

> ⚠️ В `setProperty` такие макросы передаются через параметр **`value`** (прямое значение): параметр `source` трактуется как путь (`event.value` / `state.field`), и макро-строка в нём не разрешается.
