...
**Макросы диапазонов дат** (вычисляются от локального времени браузера, возвращают ISO-строку UTC — абсолютные мгновения, корректные для любой таймзоны сервера):
