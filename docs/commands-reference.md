...
HTTP-запрос к API с записью результата в state и поддержкой коллбэков.
...
| `onSuccess` | `Command[]`    | Команды при успешном выполнении |     ❌      |
| `onError`   | `Command[]`    | Команды при ошибке              |     ❌      |
...
**Автоматическая детализация ошибок:** если тост показывается в контексте `onError`
(`sendRequest` / `downloadFile`), к сообщению автоматически добавляется полная
информация об ошибке из ApiError — `error.message` (если отличается) и построчно
`details`: `• field: issue` (до 10 строк, далее «…и ещё N»). Поэтому в конфиге
достаточно общего текста («Не удалось сохранить…») — конкретная причина и поля
с ошибками валидации попадут в тост сами:

```json
// Ошибка API: {"error":{"code":"VALIDATION","message":"Validation failed",
//   "details":[{"field":"email","issue":"Неверный формат почты"}]}}
// Текст тоста:
...
{
  "type": "showToast",
  "params": { "message": "Не удалось сохранить отчёт", "severity": "error" }
}
```
