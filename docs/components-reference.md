...
- `className` и `style` применяются к **внешнему оборачивающему `<div>`**
- `containerClassName` применяется к **внутреннему контейнеру с дочерними компонентами**
- Если задан `grid`, к `containerClassName` добавляется класс `grid`, каждый дочерний компонент оборачивается в `<div>` с `col-*` классом из `grid.cols` (по индексу)
- Если `grid` не задан — компоненты рендерятся напрямую во flex-контейнере с `containerClassName`
...
```json
{
  ...
  "componentType": "LayoutGroup",
  "props": {
    "containerClassName": "flex flex-row gap-2 align-items-center",
    "components": [
      {
        "id": "avatar",
