...
Контейнер-группировщик дочерних компонентов. По структуре и поведению аналогичен `BlockRenderer`, но работает на уровне компонента — позволяет сгруппировать несколько компонентов внутри одного слота и управлять их лэйаутом.

| Пропс        | Тип      | По умолч.                  | Описание                            |
| ------------ | -------- | -------------------------- | ----------------------------------- |
| `components` | `array`  | `[]`                       | Массив дочерних компонентов         |
| `grid`       | `object` | —                          | PrimeFlex grid-раскладка (см. ниже) |
| `className`  | `string` | `"flex flex-column gap-2"` | CSS-класс контейнера                |
...
- Если задан `grid`, контейнер получает класс `grid`, каждый дочерний компонент оборачивается в `<div>` с `col-*` классом из `grid.cols` (по индексу)
- Если `grid` не задан — компоненты рендерятся напрямую во flex-контейнере с классом по умолчанию `"flex flex-column gap-2"`
- При пустом `components` рендерится пустой `<div>`
...
Контейнер с настраиваемой меткой. Поддерживает всё, что `LayoutGroup`, плюс управление позицией и стилем метки.

| Пропс            | Тип                | По умолч.                  | Описание                                     |
| ---------------- | ------------------ | -------------------------- | -------------------------------------------- |
| `label`          | `string`           | —                          | Текст метки                                  |
| `labelPosition`  | `"top"` / `"left"` | `"top"`                    | Позиция метки                                |
...
| `className`      | `string`           | `"flex flex-column gap-2"` | CSS-класс для контейнера компонентов         |
| `style`          | `object`           | —                          | Inline-стили внешнего контейнера             |

**Поведение `labelPosition`:**

- `"top"` — метка сверху, компоненты снизу (вертикальная раскладка)
- `"left"` — метка слева с `min-width: 120px`, компоненты справа (горизонтальная раскладка)

**Особенности:**

- Если `label` не задана или пуста — ведёт себя как обычный `LayoutGroup` (рендерит только компоненты)
- `labelClassName` позволяет управлять размером, цветом, выравниванием метки (напр. `"text-lg font-bold text-primary mb-2"`)

**Пример (labelPosition: "left"):**
...
**Пример (labelPosition: "top" с grid):**
...
```json
{
  ...
  "props": {
    "label": "Контакты",
    "labelPosition": "top",
    "labelClassName": "font-semibold text-md mb-1",
    "grid": {
      "cols": ["col-6", "col-6"]
    },
    "components": [
