...
**Стабильные DOM-id:** движок автоматически передаёт в MultiSelect `id` (из `id` компонента в конфиге) и `inputId` (`{id}-input`), если они не заданы в `props`. Без них PrimeReact вычисляет `inputId` через `UniqueComponentId()` прямо в теле рендера — глобальный счётчик различается между SSR и гидратацией → hydration-mismatch на атрибутах скрытого инпута (`aria-controls="{inputId}-multi-selectbox"`).
...
1. Если `field` кастомного столбца **совпадает** с `field` одного из существующих столбцов в `columns` — существующий столбец **заменяется** кастомным (позиция сохраняется, если не задан `order`).
2. Если совпадения нет — кастомный столбец **добавляется** в конец таблицы (или по `order`); так удобно добавлять кнопки действий в строках.
...
| `headerComponents`   | `array`  | —                          | Компоненты в правой части хедера (слот `icons`)   |
...
- `headerComponents` — массив компонентов в правой части хедера (слот `icons` PrimeReact Panel); рендерятся через ComponentRenderer, поэтому биндинги и события (например, кнопка «Обновить») работают как в теле панели
