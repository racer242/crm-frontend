...
// → true если moderation_status равен "REFUSED"
```
...
| Параметр | Тип | Описание                                                     |
| -------- | --- | ------------------------------------------------------------ |
| `value`  | any | Сравниваемое значение (обычно биндинг)                       |
| `equals` | any | Эталонное значение (обе стороны приводятся к строке)         |

Типичное применение — в config-level `visible` компонента (см. components-reference, «Видимость компонента»): показать поле сразу при выборе определённого значения в дропдауне, не дожидаясь сохранения на сервер.
...
### 6. `format` — форматирование строки
