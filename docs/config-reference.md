...
> **Структура страниц**: страницы группируются по каналам/инстансам — `config/pages/promo-instance/` содержит операционные страницы промо-инстанса (участники, чеки, призы, акты, сообщения). В `pages` указывается полный относительный путь (`./pages/promo-instance/...`). Значения `$ref` внутри файла страницы резолвятся **относительно самого файла**, поэтому при переносе страницы в подпапку её внутренние ссылки поднимаются на уровень выше: `../menus/user.json` → `../../menus/user.json`.
