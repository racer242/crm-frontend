...
> **Подзаголовок (`header`)** — пункт-заголовок группы: рендерится компонентом PrimeReact `Divider` (`align="left"`, текст = значение `header`), команда перехода не навешивается, поэтому клик никуда не ведёт и пункт никогда не подсвечивается как активный. В свёрнутой панели (4rem) текст не помещается — выводится только линия-разделитель. Приоритет над `route` и `separator`.
