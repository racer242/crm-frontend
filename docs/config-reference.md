...
```json
{
  ...
  "navbar": { ... },
  "userMenu": { ... },
  "pages": [{ "$ref": "./pages/promo-instance/profile.json" }],
  "config": { ... }
}
```

> **Структура страниц**: страницы группируются по каналам/инстансам — `config/pages/promo-instance/` содержит операционные страницы промо-инстанса (участники, чеки, призы, акты, сообщения), `config/pages/crm-management/` — страницы канала CRM-управления (модерация чеков). В `pages` указывается полный относительный путь (`./pages/promo-instance/...`, `./pages/crm-management/...`). Значения `$ref` внутри файла страницы резолвятся **относительно самого файла**, поэтому при переносе страницы в подпапку её внутренние ссылки поднимаются на уровень выше: `../menus/user.json` → `../../menus/user.json`.
...
| `header`    | `string`  | Подзаголовок раздела (выделен, без перехода)  |
| `separator` | `boolean` | Показать разделитель                          |

> **Подзаголовок (`header`)** — пункт-заголовок группы: рендерится компонентом PrimeReact `Divider` (`align="left"`, текст = значение `header`), команда перехода не навешивается, поэтому клик никуда не ведёт и пункт никогда не подсвечивается как активный. В свёрнутой панели (4rem) вместо текста выводится первая буква подзаголовка (по центру). Цвет текста приглушён PrimeFlex-классом `text-color-secondary` — кастомные CSS не используются. Приоритет над `route` и `separator`.

```json
"items": [
  { "header": "Промо-инстанс" },
  { "label": "Участники", "icon": "pi pi-users", "route": "/ops/users" },
  { "separator": true }
]
```
...
| `grid`    | `GridConfig`              | Грид-раскладка с обёртками для блоков (см. GridConfig) | ❌          |
...
Настройка грид-раскладки для секции. Если `grid` указан, каждый блок секции автоматически оборачивается в `<div>` с соответствующим col-классом. Секция получает классы `grid grid-nogutter` (дополнительно к `className`).

Используется вместо CSS-свойства `gap`, которое ломает расчёт ширины у PrimeFlex колонок. Вместо `gap` отступы задаются через padding на wrapper-ах блоков.

#### Параметры

| Параметр  | Тип        | Описание                                                                   | Обязательно |
| --------- | ---------- | -------------------------------------------------------------------------- | ----------- |
| `cols`    | `string[]` | Массив col-классов для каждого блока. Порядок соответствует порядку блоков | ✅          |
| `padding` | `string`   | PrimeFlex padding-класс для всех wrapper-ов (например `"px-1"`)            | ❌          |
...
```json
{
  ...
  "type": "section",
  "grid": {
    "cols": ["col-12 md:col-10", "col-12 md:col-2"],
    "padding": "px-1"
  },
  "blocks": [
    { "id": "left", "type": "block", ... },
...
В результате рендерится:

```html
<section class="grid grid-nogutter">
  <div class="col-12 md:col-10 px-1">
    <div class="block-content">...</div>
  </div>
  <div class="col-12 md:col-2 px-1">
    <div class="block-content">...</div>
  </div>
</section>
```

> **Важно:** При использовании `grid` не нужно указывать col-классы в `block.className` или `block.wrapper.className` — они задаются только в `grid.cols`.
...
Для `wrapper.component: "Panel"` в `props` дополнительно поддерживается `headerComponents` — массив компонентов (по схеме Component), рендерящихся в правой части хедера панели (слот `icons` PrimeReact Panel); биндинги и события работают как у обычных компонентов блока. Пример: кнопка «Обновить» на панели «Профиль» страницы участника (`config/pages/promo-instance/user.json`).
...
> Примечание: режим `"refresh"` (по умолчанию) шлёт `router.refresh()` — Next.js перезапускает SSR-dataFeed страницы, и AppEngine повторно применяет свежие результаты фида в client state (сравнение по содержимому фида), поэтому содержимое страницы действительно обновляется. Несохранённые клиентские правки state сбрасываются на данные сервера — это ожидаемое поведение кнопки «Обновить». Для обновления отдельного блока данных без участия SSR используйте вложенный `sendRequest` GET в `onSuccess` (см. шорткат `saveReceipt` карточек чека). В dev-режиме это работает при отключённом `experimental.serverComponentsHmrCache` в `next.config.ts`: при включённом (по умолчанию) флаге Next кэширует ответы `fetch` SSR-фидов между HMR-обновлениями и `router.refresh()` возвращает прежние данные (кэш очищается только навигацией или полной перезагрузкой страницы).
...
### Соответствие маршрутов страниц

Маршрут страницы (`page.route`) может содержать динамические сегменты вида `[param]`. Поиск страницы по URL (`findPageByRoute`) работает так:

1. Точное совпадение URL с маршрутом какой-либо страницы.
2. Паттерн-матчинг по всем страницам: сегменты сверяются по очереди, `[param]` совпадает с любым сегментом и попадает в `{$location.routeParams.param}`.

Если одному URL подходит несколько шаблонов (например, `/ops/users/1/acts/add` подходит и шаблону карточки акта `/ops/users/[user_id]/acts/[act_id]`, и странице добавления `/ops/users/[user_id]/acts/add`), выбирается **наиболее специфичный** — шаблон с наибольшим числом совпавших статичных сегментов. Порядок регистрации страниц в `crm-config.json` на результат не влияет (используется только как тай-брейкер при равной специфичности).
