...
- 9 типов форматирования (Date, Time, Number, Currency, RelativeTime, List, Plural, RemoveProps, PopIfHasNullProps)
...
| `RemoveProps`       | Удаление свойств из объекта/массива  |
| `PopIfHasNullProps` | Фильтрация массива по null свойствам |
...
| `ListFormat`         | List                 |
| `PluralRules`        | Plural               |
| _(не используется)_  | RemoveProps          |
...
## 7. RemoveProps — удаление свойств

Удаление перечисленных свойств из объекта или массива объектов. В отличие от других типов форматирования, не использует Intl API.

### Поведение

| Входное значение   | Результат                                         |
| ------------------ | ------------------------------------------------- |
| `Date`             | Возвращается без изменений                        |
| `null`/`undefined` | Возвращается без изменений                        |
| Примитив           | Возвращается без изменений                        |
| Массив             | Удаление применяется ко всем элементам рекурсивно |
| Объект             | Удаляются перечисленные свойства                  |
...
## 8. PopIfHasNullProps — фильтрация массива по null свойствам

Удаляет элементы массива, у которых хотя бы одно из указанных свойств равно `null` или `undefined`. В отличие от других типов форматирования, не использует Intl API.

### Поведение

| Входное значение | Результат                                      |
| ---------------- | ---------------------------------------------- |
| Не массив        | Возвращается без изменений                     |
| Массив           | Элементы с null/undefined свойствами удаляются |

### Параметры

`params` — объект с полем `props`, содержащим массив строк с путями к проверяемым свойствам. Поддерживаются вложенные пути через точку.

### Примеры

```json
{
  "type": "PopIfHasNullProps",
  "func": "DateTimeFormat",
  "params": { "props": ["email"] }
}
// → [{ id: 1, email: "a@b.com" }, { id: 2, email: null }]
// → [{ id: 1, email: "a@b.com" }]

{
  "type": "PopIfHasNullProps",
  "func": "DateTimeFormat",
  "params": { "props": ["name", "email"] }
}
// → [{ name: "A", email: "a@b.com" }, { name: "B", email: null }, { name: null, email: "c@d.com" }]
...
| `RemoveProps`       | _(не используется)_  | object, array        | object    | `{ id: 1 }` (без `secret`) |
| `PopIfHasNullProps` | _(не используется)_  | array                | array     | `[{ email: "a@b.com" }]`   |
