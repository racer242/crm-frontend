...
- **Клиентское форматирование дат в таблицах (шаг 2, применено ко всем таблицам)** (`src/utils/dateFormat.ts`, `src/engine/components/DataTableComponent.tsx`, 18 адаптеров): колонки с документированным маркером PrimeReact `dataType: "date"` рендерятся через `body`-функцию с компонентом `DateCell` — дата форматируется **в часовом поясе зрителя** после гидратации (SSR-безопасно: сервер и первый клиентский рендер показывают сырое значение, без hydration-mismatch). Паттерн задаётся в конфиге колонки (`dateFormat`, токены DD/MM/YYYY/HH/mm/ss), по умолчанию `DD.MM.YYYY HH:mm`. Маркер расставлен: в адаптерах с явными колонками (`users.get`, `user-acts`, `user-messages`, `acts`, `receipts`, `user-receipts`, `messages`) и автоматическим маппингом `type: datetime/date` → `dataType: date` в адаптерах формата columns/rows (11 шт.: `acts.get`, `codes`, `events`, `gtins`, `messages.get`, `mgmt.receipts.get`, `points-history`, `prizes.get`, `products`, `receipts.get`, `transactions`) и в колонках результата статистики (`stats.report.get.response.js`, из `fields[].type`). Адаптеры даты по-прежнему не форматируют — только передают сырой ISO и метаданные колонок. Эксперименты с `type: "datetime"` (неизвестный проп, игнорируется) и «голым» `dataType` (влияет только на режимы фильтра) — без эффекта, см. предыдущие коммиты.
