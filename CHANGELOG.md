...
- **Доработки карточек-детейлов и списков сообщений**:
  ...
  - Попап сообщения доработан: каждое поле — отдельный компонент `Text` с одним биндингом (`message_id`, `sender_name_label`, `sender_email_label`, `status_label`, `created_at_formatted`); кнопки — «Написать письмо» (команда `navigate` с готовым `mailto:{email}?subject=…` из `mailto_link` адаптера, активна при наличии e-mail) и «Закрыть». Строки таблиц сообщений активны, как в остальных списках (`selectionMode: "single"`).
