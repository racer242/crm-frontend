...
- **Доработки карточек-детейлов и списков сообщений**:
  ...
  - Попап сообщения — форма на компонентах движка: блок отправителя (`Avatar` с инициалами и фолбэк-иконкой + имя/e-mail), разделители `Divider`, «Статус» (`Tag`, severity из `status_severity`: delivered → success, failed → danger) и «Дата» в две колонки, «Код обращения» (перенос длинного UUID), полный текст «Сообщение» с переносами строк; подписи полей — `LabelledGroup`, раскладка — `LayoutGroup` (несуществующий компонент `Container` убран). Кнопки: «Написать письмо» (команда `navigate` с готовым `mailto:{email}?subject=…` из `mailto_link` адаптера, активна при наличии e-mail) и «Закрыть», выровнены вправо. Строки таблиц сообщений активны, как в остальных списках (`selectionMode: "single"`).
