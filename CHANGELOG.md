...
- **Повторная доставка того же SSR-фида не применялась, если state ушёл вперёд клиентскими изменениями** (`src/engine/AppEngine.tsx`): сценарий — «Автомодерация» установила причину X (применено), затем «Применить» записал в `state.receiptData` пользовательскую причину Y (вложенный клиентский GET), затем «Автомодерация» снова вернула X: сигнатура нового фида совпадала с последней применённой (`feedChanged: false`), и данные не применялись — на экране оставался Y, хотя сервер уже вернул X. Сравнение с «последней применённой сигнатурой» заменено сравнением каждого результата фида с **текущим состоянием** (`getStateField`/`getState` + `JSON.stringify`): применяются только отличающиеся результаты; сигнатура-реф и `initialFeedSignature` удалены. Побочные эффекты новой логики: StrictMode-двойной запуск эффекта больше не дублирует запись (после первого применения данные равны), повторная доставка идентичного фида не дёргает перерисовку.
- **`router.refresh()` применял фид в пустую на страницах с динамическим сегментом маршрута** (`src/engine/AppEngine.tsx`): эффект ре-гидрации резолвил страницу по реальному URL (`resolveRouteWithFallback(pathname)`), а `StateManager.getPageByRoute` сравнивает строго с шаблоном (`/mgmt/receipts/[receipt_id]`) — для карточек с `[param]` `newPageId` получался `null`, `targetId = elementId || newPageId = null`, и все результаты фида пропускались (`if (!targetId) continue`), хотя клиент их получал (`feedChanged: true`). На навигации дефект не проявлялся: там `AppEngine` перемонтируется и фид применяет конструктор `StateManager` с серверным `initialPageId`. Резолв страницы заменён на серверный маршрут (`initialRoute ?? resolveRouteWithFallback(pathname)`) с фолбэком на `initialPageId`. Локализовано по dev-диагностике `[SSR feed]` (сервер) / `[AppEngine feed]` (браузер) — оставлена в dev-режиме.
- **Не работало обновление страницы по команде `refresh` в dev-режиме** (`next.config.ts`, `src/core/DataFeedServerService.ts`): в Next.js 16 экспериментальный флаг `serverComponentsHmrCache` по умолчанию включён и кэширует ответы `fetch` в Server Components между HMR-обновлениями; кэш очищается только при навигации или полной перезагрузке страницы, а `router.refresh()` (команда `refresh` — кнопки «Обновить», `onSuccess` «Автомодерации» и сохранения продукта) не является ни тем, ни другим — SSR-dataFeed возвращал прежние данные, сигнатура фида в `AppEngine` не менялась, состояние не переприменялось. Поэтому «Обновить» не давали эффекта (при этом «Применить» работал — там вложенный клиентский GET мимо SSR). Исправление: `experimental.serverComponentsHmrCache: false` в `next.config.ts` (по доке этой версии Next) + явный `cache: "no-store"` для запросов SSR-фидов. Задокументировано в `README.md` (раздел «Конфигурация Next.js») и `docs/config-reference.md`.

### Added

- **Модерация продуктов чека: редактирование в попапе** (`config/pages/crm-management/receipt.json`, новый request-адаптер `config/adapters/mgmt.receipt.patch.request.js`, `config/system/adapters.json`, `config/system/api-routes.json`, `docs/components-reference.md`): в таблице продукции на странице модерации чека добавлена кнопка-карандаш в каждой строке (новая кастом-колонка `edit`); клик открывает модальный Dialog «Редактирование продукта» — название (read-only), переключатель «Акционный» (`is_promo`) и Dropdown «Статус продукта» (PENDING/CONFIRMED/CANCELLED). «Сохранить» шлёт PATCH `api/receipts/{id}` с телом `products: [{gtin, is_promo, product_status}]` (§5.4; `product_status` в доке пока не зафиксирован — включён по договорённости с бэкендом, при отказе убирается в адаптере; обязательный `moderation_status` отправляется текущим round-trip'ом, как у «Применить»); после успеха — закрытие попапа, toast, перезагрузка таблицы продукции (`requestProducts` с текущей пагинацией) и деталей чека (промо-агрегаты `promo_products_count/amount`). «Отмена» и крестик просто закрывают попап (`onHide`). После успешного сохранения — обновление страницы (команда `refresh`: SSR-dataFeed перезапускается, свежие данные применяются в client state). Request-адаптер `mgmt.receipt.patch.request` прикреплён к PATCH-маршруту: тела без `products` (кнопка «Применить») проходят без изменений; с `products` — `is_promo` нормализуется к boolean (макросы строк дают `"true"/"false"`, полный макрос `{$state...}` сохраняет тип), `product_status` включается только непустым. Тумблер читает значение через реактивный calc `if(equals(...))` — корректно отображает и строку, и boolean. Заодно поправлено описание `customColumns` в `docs/components-reference.md`: колонка без совпадения `field` не игнорируется, а добавляется в конец таблицы (код работал именно так). Проверено автотестом (JSON-структура + исполнение адаптера на синтетических телах).

- **Реактивная видимость по значению поля: calc-операция `equals` и calc-объект в `visible`** (`src/core/CalcEngine.ts`, `src/engine/hooks/useComponentBindings.ts`, `src/types/component.ts`, `config/pages/crm-management/receipt.json`, `docs/calc-reference.md`, `docs/components-reference.md`): config-level `visible` компонента теперь принимает не только boolean/binding-строку, но и **calc-выражение** — `Linkage.resolveDeep` выполняет его, биндинги внутри `params` реактивны (подписки на state строятся по тем же правилам, что и для props; тип `Component.visible` расширен до `CalcOperation`). В CalcEngine добавлена операция **`equals`** — сравнение двух значений с приведением к строке. На странице модерации чека панель «Причина отклонения» теперь показывается **сразу при переключении статуса на «Отклонен»** (REFUSED) и скрывается при любом другом: `visible` = `{"calc":"equals","params":{"value":"@state.receiptData.moderation_status","equals":"REFUSED"}}` — привязка к выбранному в дропдауне значению, а не к флагу `show_decline_reason`, посчитанному адаптером из загруженных с сервера данных (этот флаг остался для отображения текущей причины в информационной панели). При загрузке уже отклонённого чека поле показывается как раньше. Проверено автотестом структуры конфига и `tsc --noEmit` (exit 0).

- **Кнопки «Обновить» работают на всех страницах** (`src/engine/AppEngine.tsx`): команда `refresh` шлёт `router.refresh()`, сервер честно перезапускал SSR-dataFeed и передавал свежий `initialDataFeed` в клиент, но эффект ре-гидрации в AppEngine применял его **только при смене страницы** — данные той же страницы отбрасывались, и содержимое не обновлялось. Теперь фид применяется и при изменении его содержимого (сигнатура `JSON.stringify` фида в ref): повторные рендеры с теми же данными пропускаются, клиентская навигация работает как прежде; осознанный побочный эффект — «Обновить» перечитывает данные SSR, сбрасывая несохранённые клиентские правки state. Побочно починилась и «Автомодерация» на странице модерации чека (её `onSuccess`/`onError` используют команду `refresh`).

- **Кнопка «Применить» обновляет карточку чека участника и страницу модерации чека** (`config/pages/promo-instance/user-receipt.json`, `config/pages/crm-management/receipt.json`): в `onSuccess` PATCH шортката `saveReceipt` добавлен вложенный GET-запрос, перезагружающий `state.receiptData` — по образцу общей карточки чека (`/ops/receipts/[receipt_id]`), где это уже было и работало; статус и причина отклонения обновляются на экране сразу после сохранения, без перезагрузки страницы.

- **Убраны бесполезные поля ввода в карточках чека**: «Причина отклонения» (InputTextarea) — из панелей «Модерация» общей карточки чека (`config/pages/promo-instance/receipt.json`) и карточки чека участника (`config/pages/promo-instance/user-receipt.json`); «Комментарий» (InputTextarea) — со страницы модерации чека (`config/pages/crm-management/receipt.json`). PATCH в `saveReceipt` по-прежнему отправляет текущие значения этих полей из state (round-trip загруженных данных) — содержимое не теряется; отображение причины отклонения на модерации (`show_decline_reason`, `decline_reason_label`) не затронуто. Проверено автотестом структуры (21 проверка: JSON всех трёх конфигов, вложенный GET в `saveReceipt`, отсутствие удалённых компонентов, целостность кнопок/дропдаунов) и `tsc --noEmit` (exit 0).

- **История баллов участника** (`config/pages/promo-instance/user-points-history.json` — новая; адаптеры `user-points-history.get.request.js` / `user-points-history.response.js` — новые; `config/system/api-routes.json`, `config/system/adapters.json`, `config/menus/user.json`, `config/crm-config.json`): страница «История начисления и списания баллов» (`/ops/users/[user_id]/points/history`, §6.3 `GET /users/[id]/points/history`, page/limit + фильтр `type`) — крошки (Участники → участник → «Баллы»), меню участника: **«Баллы»** (`pi pi-wallet`) в подменю группы **«Достижения»** (рядом с «Призы»), на странице — `activeItemId: "achievements"` + `activeSubitemId: "points"`, заголовок с кнопкой «Обновить», фильтр «Тип операции» (Dropdown «Все типы» + опции из фактических данных ответа — уникальные `type_code`/`type_name`, домен кодов в доках не зафиксирован; значение синхронизируется с URL через `dataInit` + `mergeUrlParams`), lazy-таблица (ID транзакции uuid, Дата, Тип, Изменение со знаком +N/−N, Комментарий «—»), пагинация с URL-синхронизацией; request-адаптер с event-слиянием `first/rows` (паттерн `users.get.request`). Маршрут — в `api-routes.json`, адаптеры — в `adapters.json`, страница — в `crm-config.json`; проверено автотестом (23 проверки).

- **Кнопки-переходы на счётчиках карточки участника** (`src/engine/components/CardComponents.tsx`, `src/app/globals.css`, `config/pages/promo-instance/user.json`): StatCard получил опциональный проп `actionIcon` — кнопка-иконка в правом нижнем углу карточки (настоящий `<button>` с aria-label «Перейти: {title}», круглый hover-фон через класс `stat-card-action`; сама карточка статична), клик шлёт `onClick` через штатный `handleEvent` движка; цвет иконки — `actionIconColor` (по умолчанию `var(--surface-400)`). На карточке участника стрелки ведут: «Чеки/все» → `/ops/users/{id}/receipts`, «Чеки/приняты» → `.../receipts?status=ACCEPTED` (фильтр подхватывается из URL штатной синхронизацией страницы чеков участника), «Продукты» → `.../products`, «Сообщения» → `.../messages`, «Призы» → `.../prizes` (цвет иконки под primary-фон — `var(--primary-color-text)`); «Баллы» — без перехода. Без `actionIcon` StatCard рендерится как раньше — остальные страницы не затронуты. Задокументировано в `docs/components-reference.md`; проверки автотестом (13) и `tsc --noEmit` (exit 0).

- **Карточка розыгрыша: просмотр шансов + правка раскладки** (`config/pages/promo-instance/raffle.json`, новые адаптеры `config/adapters/raffle.chances.get.request.js` / `raffle.chances.response.js`, `config/system/api-routes.json`):
  - панель «Шансы розыгрыша» (§8.9 `GET /raffles/[id]/chances`, page/limit) — lazy-таблица с пагинацией (`requestRaffleChances`, селектор 5/10/25/50, ошибка — toast); колонки строятся из **фактических полей ответа** («все актуальные поля шанса»): известные поля (`chance_id`, `user_id`, `received_at`, `source_type`, `raffle_id`, `is_winning`, `is_winner`, `booked_at`, `status`) получают русские заголовки и типы (uuid — сокращённый формат, date — форматирование в поясе зрителя), дополнительные поля API добавляются в конец с заголовком из имени ключа; логические значения — Да/Нет, null — «—», источники шансов — лейблами справочника (receipt → Чек и т.д.);
  - кнопка «Редактировать» перенесена из отдельной секции в низ панели «Информация о розыгрыше» — под `Divider` по образцу карточки участника; поведение прежнее (navigate на `/edit`, блокировка по `edit_disabled`); `align-self-start` — кнопка прижата к левому краю и не растягивается на всю ширину (блок — `flex flex-column`, в отличие от панели участника с `align-items-center`);
  - маршрут `ops/raffles/[raffle_id]/chances` GET → `/api/v1/crm/raffles/{id}/chances`, адаптеры зарегистрированы в `config/system/adapters.json`; проверено автотестом (24 проверки: структура конфига, пагинация, форматирование, конверты).

- **Раздел «Розыгрыши» промо-инстанса** — работа с розыгрышами (`/api/v1/crm/raffles`, §3.5 / §8):
  - `config/menus/navbar.json` — пункт «Розыгрыши» (`pi pi-trophy`, `/ops/raffles`) сразу после «Акты»;
  - `config/pages/promo-instance/raffles.json` (новая, `/ops/raffles`) — список розыгрышей: lazy-таблица (Название, ID uuid, Статус Tag, Источник шансов, Срок сбора from–to, Дата проведения date, Публикация Да/Нет), заголовок с кнопкой «Обновить», пагинация `requestRaffles` + URL-синхронизация, клик по строке → карточка розыгрыша; адаптеры `raffles.get.response` (конверт → таблица; лейблы статусов `draft/in_progress/completed/published/annulled` и источников шансов `receipt/promo_code/gtin_code/game_win/points_purchase/manual`, период `{from,to}` → «DD.MM.YYYY – DD.MM.YYYY») и `raffles.get.request` (first/rows → page/limit);
  - `config/pages/promo-instance/raffle.json` (новая, `/ops/raffles/[raffle_id]`) — карточка розыгрыша: панель «Информация о розыгрыше» с кнопкой обновления в хедере (`headerComponents`), панель «Протокол розыгрыша» с кнопкой «Скачать протокол» (`downloadFile` GET `ops/raffles/protocol?raffle_id=` → §8.7, JSON-файл `protocol_{id}.json`), кнопка «Редактировать»; адаптер `raffle.get.response` (детали → плоский state: сырые `name`/`chance_source_type`/`collection_from`/`collection_to` для формы + `*_label`/`*_severity` для просмотра);
  - `config/pages/promo-instance/raffle-edit.json` (новая, `/ops/raffles/[raffle_id]/edit`) — редактирование (§8.4, PATCH): Название (InputText), Источник шансов (Dropdown, 6 опций), Срок сбора (два Calendar showTime) → request-адаптер `raffle.patch.request` собирает тело с вложенным `collection_period:{from,to}`; кнопки «Опубликовать» (§8.5, POST `ops/raffles/publish`, confirm) и «Аннулировать» (§8.6, POST `ops/raffles/annul`, confirm с предупреждением о сбросе шансов) с возвратом на карточку; «Сохранить»/«Применить»/«Отменить изменения» — по образцу act-edit;
  - `config/system/api-routes.json` — 6 маршрутов instance-канала: `ops/raffles` GET, `ops/raffles/publish` POST, `ops/raffles/annul` POST, `ops/raffles/protocol` GET (raffle_id в query), `ops/raffles/[raffle_id]` GET/PATCH — литеральные пути объявлены раньше `[raffle_id]`, чтобы не перехватывались; адаптеры зарегистрированы в `config/system/adapters.json`, страницы — в `config/crm-config.json`.

- **«Всего: N» на панели пагинации DataTable** (`src/engine/components/DataTableComponent.tsx`): при `paginator: true` и известном `totalRecords` в конец шаблона пагинатора (после селектора строк) автоматически добавляется элемент `CurrentPageReport` с текстом «Всего: {totalRecords}»; остальной вид пагинатора не меняется. Переопределяется пропсами страницы `paginatorTemplate`/`currentPageReportTemplate`; при отсутствии `totalRecords` надпись не выводится (статичные таблицы в карточках не затронуты). Задокументировано в `docs/components-reference.md`.
- **Курсор надписи «Всего: N»** (`src/app/globals.css`): `.p-paginator .p-paginator-current { cursor: default }` — надпись не интерактивна, но наследовала `cursor: pointer` от элементов пагинатора.

- **Поиск и фильтры на странице «Чеки»** (`config/pages/promo-instance/receipts.json`, `config/adapters/ops-receipts.get.request.js`): секция фильтров по образцу страницы участников — `InputTextWithThrottle` «Быстрый поиск» + `Dropdown` статуса модерации (CHECKING/ACCEPTED/REFUSED/SUSPENDED/NEED_PHOTO/HAND_LOAD) + `Dropdown` статуса ФНС (no_check/wait/wrong/correct); параметры `search`/`status`/`fns_status` уходят в запрос, синхронизируются с URL (`dataInit` + `mergeUrlParams`). Общий список чеков 2.11 переведён со shared-адаптера участников на **отдельный request-адаптер `ops-receipts.get.request`** (page/limit; пробрасывает также `user_id` — счётчики на карточке участника продолжают работать), чтобы параметры участников и чеков не смешивались.
- **Поиск и фильтр по теме на странице «Сообщения»** (`config/pages/promo-instance/messages.json`, `config/adapters/users.get.request.js`, `config/system/api-routes.json`, `config/adapters/dictionaries.response.js`): `InputTextWithThrottle` «Быстрый поиск» + `Dropdown` «Тема...» (options из справочника тем); параметры `search`/`topic` в запросе и URL. Справочник тем загружается SSR-фидом через новый API-маршрут `ops/dictionaries/[name]` GET → `GET /api/v1/crm/dictionaries/[name]` (§3.6.3, instance-канал); адаптер `dictionaries.response` разворачивает конверт и превращает `items[]` (`code`, `name`, `is_active`) в options Dropdown (`{label, value}`), неактивные темы видны с пометкой «(неактивна)». Request-адаптер `users.get.request` дополнительно пробрасывает `topic` (только если непустой — другие маршруты не затронуты).
- **Заголовок страницы модерации чеков** (`config/pages/crm-management/receipts.json`): H1 «Чеки» → «Модерация чеков» (в соответствии с `meta.title` и пунктом меню).
...
- **`headerComponents` — компоненты в хедере Panel** (`src/engine/components/ContainerComponents.tsx`, `src/engine/BlockRenderer.tsx`): новый опциональный проп для компонента `Panel` и для Panel-обёртки блока (`wrapper.props.headerComponents`) — JSON-массив компонентов, рендерящихся в правой части хедера панели через слот `icons` PrimeReact Panel тем же ComponentRenderer (биндинги и события работают как в теле). При отсутствии пропа поведение панелей не меняется. Задокументировано в `docs/components-reference.md` и `docs/config-reference.md`.
- **Кнопка «Обновить» на панели «Профиль» страницы участника** (`config/pages/promo-instance/user.json`): иконка-кнопка (`pi pi-refresh`, tooltip) в правом верхнем углу хедера панели через новый `headerComponents`, команда `refresh` — обновляет SSR-данные карточки (профиль + счётчики).
- **Кнопка «Обновить» на основных панелях страниц детального просмотра** (шаблон с `className: "-m-3"` — кнопка прижата к правому краю хедера): `receipt.json`/`user-receipt.json` — панель «Информация о чеке», `act.json`/`user-act.json` — «Информация об акте», `prize.json`/`user-prize.json` — «Информация о призе», `user-docs.json` — «Документы», `crm-management/receipt.json` — «Информация о чеке» (mgmt-канал); команда `refresh`. Формы/редактирование (`act-add/act-edit`, `user-act-add/user-act-edit`, `user-docs-edit`, `user-edit`), список статистики, `stats-report.json` и `profile.json` — без изменений.
- **IP и User-Agent в попапе события журнала** (`config/pages/promo-instance/user-log.json`, `config/adapters/user-log.response.js`): над «Данными события» добавлены группы «IP-адрес» и «User-Agent» (`ip_address_label`/`user_agent_label`, фолбэк «—» при отсутствии); длинный User-Agent переносится штатным `wrap-anywhere` компонента Text.
- **Колонка «IP-адрес» в списке событий журнала** (`config/adapters/user-log.response.js`): колонка «Данные события» (`payload_summary`) заменена на «IP-адрес»; генерация `payload_summary` удалена (полный payload по-прежнему доступен в попапе как `payload_pretty`).
...
- **Кнопка «Обновить» в заголовках всех списков** (12 страниц `config/pages/promo-instance/`: чеки, призы, акты, сообщения, статистика ×2, чеки/акты/призы/покупки/сообщения/журнал участника + `mgmt/receipts` CRM-управления): секции заголовков приведены к единому эталону страницы «Участники» — блок `w-full flex gap-3 align-items-center`, заголовок `w-full`, справа иконка-кнопка «Обновить» (`pi pi-refresh`, tooltip, команда `refresh`). На странице «Акты участника» секция `actsHeader` с кнопкой «Добавить акт» сохраняет раскладку `justify-content-between` (кнопки «Обновить» и «Добавить акт» — по правому краю, обновление рядом с заголовком).

### Fixed

- **Пагинация списка розыгрышей застревала на первой странице** (`config/adapters/raffles.get.request.js`): адаптер читал `first/rows` только из state, а при смене страницы DataTable присылает свежие значения в `event` (эталон — `users.get.request`) → запрос всегда уходил за первой страницей. Добавлено слияние `event.first ?? state.first`; тот же паттерн заложен в новый `raffle.chances.get.request`.
...
- **Карточка розыгрыша: вывод всех полей API v2 и панель «Управление»** (`config/adapters/raffle.get.response.js`, `config/pages/promo-instance/raffle.json`, `config/system/api-routes.json`, `config/system/adapters.json`):
  - «Информация о розыгрыше» показывает все поля актуального ответа GET `/api/v1/crm/raffles/{id}`: ID розыгрыша, Название, Тип, Статус (Tag), Дата сбора шансов — начало/конец (плоские `collection_from`/`collection_to`, в старых ответах — вложенный `collection_period`), Типы источников шансов (`source_types` — массив кодов с лейблами; в старых ответах — `chance_source_type`), Дата начала/конца розыгрыша (`start_at`/`end_at`), Дата публикации (`publish_at`), Дата проведения (`drawn_at`), Опубликован (`is_published`), Признак «завис» (`is_stale`), Дата создания/изменения (`created_at`/`updated_at`), Забронировано шансов (`booked_chances`), Причина аннулирования (`annul_reason`); null-значения выводятся как «—», даты форматируются на клиенте (`dataType: date`, пояс зрителя);
  - панель «Протокол розыгрыша» переименована в «Управление»: кнопки Опубликовать (§8.5), Начать проведение, Закончить проведение, Посчитать шансы, Черновик, Аннулировать (§8.6) и «Скачать протокол» (§8.7); кнопки статусов — с confirm, после успешного действия карточка перечитывается шорткатом `reloadRaffle` (GET → `state.raffleData`);
  - новые маршруты-заглушки по образцу publish/annul (пути предварительные — уточнить у бэкенда): `ops/raffles/start` POST, `ops/raffles/finish` POST, `ops/raffles/draft` POST (raffle_id в body) и `ops/raffles/chances/count` GET (raffle_id в query) с адаптером `raffle.chances.count.response` (§8.10 — число шансов под условиями, выводится в toast);
  - кнопка «Редактировать» блокируется по `is_editable` (адаптер отдаёт `edit_disabled: !is_editable`);
  - адаптер `raffle.get.response` переписан: все поля + пары `поле`/`поле_label` (ISO или «—»), толерантность к старым именам (`raffle_id`, `conducted_at`, `published`, `chance_source_type`, `collection_period`); проверено 35 автопроверками на данных из живого ответа и старой формы API.

- **Страница редактирования розыгрыша приведена к образцу user-edit** (`config/pages/promo-instance/raffle-edit.json`, `config/adapters/raffle.patch.request.js`): хлебные крошки — Розыгрыши → `{name}` (ссылка на карточку) → «Редактирование»; панель «Информация о розыгрыше» убрана — осталась только форма из двух панелей («Параметры розыгрыша»: Название, Тип (InputText), Типы источников шансов (MultiSelect по `source_types`, 6 опций); «Даты розыгрыша»: Начало/Конец сбора и Дата публикации — Calendar showTime); кнопки «Опубликовать»/«Аннулировать» перенесены на карточку (шорткаты `publishRaffle`/`annulRaffle` удалены, остались «Сохранить»/«Применить»/«Отменить изменения»); PATCH-адаптер отправляет `name`, `type` (пустая строка → `null` — тип можно очистить), `source_types` (массив), `publish_at` и плоские `collection_from`/`collection_to` — по форме актуального GET (вложенный `collection_period` больше не собирается).
...
- **Карточки чеков: вывод всех полей API** (`config/adapters/receipt.get.response.js`, `config/pages/promo-instance/receipt.json`, `user-receipt.json`): добавлены отсутствующие поля — «Статус ФНС» (Tag: label/severity по словарю §4.2 — correct→«Чек корректный»/success, wait→warning, wrong→danger, no_check→secondary), «Продуктов» (`total_products`), «Акционных» (`promo_products`); на общей карточке additionally «Дата покупки» (`date`, клиентское форматирование `dataType: date` — была только на карточке участника). Порядок строк унифицирован: ID, Статус, Статус ФНС, Сумма, Дата покупки, Продуктов, Акционных, Зарегистрирован, ФН, ФД, ФП, фото. `date_raw` не выводится (дубль `date`).

- **Карточка mgmt-чека: детали по ТЗ ред. 6** (`config/adapters/mgmt.receipt.get.response.js`, `_shared.js`, `config/pages/crm-management/receipt.json`): добавлены поля детального чека §5.2/§6.2 — «ID чека (пром.)» (receipt_id), «Статус ФНС» (Tag, label из справочника fns_statuses §4.2: no_check→secondary, wait→warning, wrong→danger, correct→success), «ИНН магазина», «Адрес магазина» (фолбэк «—»); суммы переведены на честный формат рублей (mgmt API отдаёт рубли, §2 — новый общий хелпер formatRubles без деления на 100; прежний formatSum делил на 100 и завышал значения); даты — клиентское форматирование (dataType: date, покупка — DD.MM.YYYY); «Причина отклонения» (label из decline_reasons §4.3) видима только при статусе rejected (реактивная visible), в панели «Модерация» добавлен Dropdown выбора причины, PATCH отправляет decline_reason (обязателен при rejected, §5.4); убраны легаси-поля user_name/user_email (CRM их не хранит — только user_id).

- **Карточка mgmt-чека объединена с редактированием** (`config/pages/crm-management/receipt.json`, `receipt-edit.json` удалена, реф убран из `crm-config.json` — 32→31): в правой колонке (LayoutGroup, по образцу карточки `/ops/receipts/{id}`) панели «Участник» (ID, имя и e-mail из SSR-цепочки `GET /api/ops/users/{$state.receiptData.user_id}` → `state.participantData`, кнопка «Перейти») и «Модерация» (Dropdown торговой сети и статуса, комментарий, кнопки «Автомодерация» и «Применить»); view-поля торговой сети/статуса/комментария из левой панели убраны (заменены контролами), из нижних кнопок удалена «Модерировать»; шорткаты `saveReceipt` (PATCH) и `autoModerate` (POST /check) перенесены из edit-страницы, state дополнен `saving/saved/sentSuccessfully/participantData`.

- **Список модерации чеков (mgmt): кнопка «Участник»** (`config/pages/crm-management/receipts.json`, `config/adapters/mgmt.receipts.get.response.js`): добавлена кастомная колонка с кнопкой-иконкой участника (по образцу списка `/ops/receipts`) — переход на карточку `/ops/users/{#row.user_id}`; колонка `user_id` из таблицы убрана (заменена кнопкой), значение `user_id` в строках данных сохранено для навигации.

- **Список модерации чеков (mgmt)** (`config/adapters/mgmt.receipts.get.response.js`, `config/pages/crm-management/receipts.json`): UUID-колонки `receipt_id` и `user_id` (§5.1 ТЗ CRM-бэкенда) выводятся сокращённо (`01a0...afef`) — в маппинге колонок добавлены маркеры `dataType: "uuid"` (по известным id + по `type` колонки от API, маркер даты сохранён); удалена кастомная колонка участника (кнопка-e-mail с `mailto`) — по ТЗ ред. 6 список приходит только с `user_id`, имён/email в нём больше нет; осталась только стрелка перехода к карточке чека.

- **SSR-цепочки фидов: state доступен последующим фидам** (`src/core/DataFeedServerService.ts`, `config/pages/promo-instance/receipt.json`): раньше `{$state.*}` в URL фида на сервере резолвился в пустоту (в `serverSources` не было `stateManager`, а результаты фидов применялись только после выполнения всех). Теперь `executeServerDataFeeds` ведёт временный state страницы: каждый успешный фид сразу применяется в него (через `PathResolver.parseTarget`), и каждый следующий фид резолвит макросы с шимом-доступом к этому состоянию. Это позволяет строить цепочки «объект → связанные данные» целиком на SSR. Карточка чека переведена на цепочку: фид `state.receiptData` → фид `/api/ops/users/{$state.receiptData.user_id}` → `state.participantData` — ФИО и e-mail участника приходят уже в первом SSR-рендере; page-level `events.onLoad` с клиентским `sendRequest` из предыдущей итерации удалён как ненужный.

- **Панель «Участник» на карточке чека — данные и переход** (`config/pages/promo-instance/receipt.json`): добавлен запрос данных участника — page-level `events.onLoad` → `sendRequest` `/api/ops/users/{@state.receiptData.user_id}` (маршрут с адаптером `user.get.response`) → `state.participantData` (SSR-фид не годится: URL второго фида не видит данные первого — результаты применяются после выполнения всех фидов). В панели теперь: ID участника (обычный текст — прежний «линк» на Text не работал, у Text нет событий), Имя (`participantData.full_name`), E-mail (`participantData.email`) и кнопка «Перейти» (outlined, → `/ops/users/{user_id}`). Карточка чека участника не менялась — там профиль участника уже загружается своим фидом.

- **Конвертация денежных сумм из копеек в рубли во всех адаптерах** (`_shared.js` + 4 адаптера, `config/pages/crm-management/receipt.json`): общая функция `formatSum` вынесена в `_shared.js` (копейки → «1 234,56 ₽», локаль ru-RU, null/пусто → «—»); конвертацию получили: продукция в чеке (`receipt-products.response.js` — колонка «Сумма» → `amount_label`), покупки участника (`user-products.response.js` — «Сумма» → `total_amount_label`), карточка чека CRM-управления (`mgmt.receipt.get.response.js` — `total_amount_label`/`promo_products_amount_label`, биндинги страницы переключены с сырых полей). Сумма чека (`receipt.get.response.js` — `sum_label`) работала и раньше — её локальная реализация заменена на общую. Не конвертируются: `price` призов (это баллы, не деньги) и результат отчётов статистики (типы полей задаёт отчёт).

- **Переход на участника и чип «неавторизован» в попапе сообщений** (`config/pages/promo-instance/messages.json`, `config/adapters/messages.response.js`): имя отправителя в шапке попапа при наличии `user_id` — кнопка-ссылка (Button `link: true`, паттерн событийных кнопок движка — Text события не обрабатывает) с `onClick → navigate` `/ops/users/{user_id}`; при `user_id === null` имя — обычный текст, под e-mail — `Tag` severity warning «неавторизован». Управление — реактивной `visible` (биндинги `@state.messagesDialog.user_id` / `sender_unauthorized`); адаптер добавил в строку флаг `sender_unauthorized` (`user_id` уже приходил через `...message`). Попап на странице участника (`user-messages.json`) не менялся — отправитель и есть текущий участник.

- **Счётчики на карточке участника** (`config/pages/promo-instance/user.json`, `config/adapters/users.get.request.js`): StatCard «Чеки/все» (2.16, все чеки участника) и «Чеки/приняты» (общий список чеков 2.11 с фильтрами `user_id` + `status=ACCEPTED`), «Продукты», «Сообщения», «Призы» заполнены реальными числами — SSR-dataFeed'ы с `rows=1` берут счётчик из `pagination.total_items` списков (response-адаптеры кладут его в `totalRecords`); биндинги переключены с пустых legacy-узлов (`productData`/`feedbackData`/`achevementsData` — удалены) на `state.allReceiptsStats/receiptsStats/productsStats/messagesStats/prizesStats.totalRecords`; сетка статистики переведена на 4 колонки (`col-6 lg:col-3`); у «Продуктов» подпись «акционные» убрана (2.17 не имеет фильтра по акционным — счётчик считается по всем продуктам); для фильтра 2.11 request-адаптер `users.get.request` теперь пробрасывает `user_id` (остальные маршруты не передают его — поведение не изменилось). «Баллы» — как раньше, из профиля.

- **Исправлен рендер правой колонки карточки чека участника** (`config/pages/promo-instance/user-receipt.json`): у `LayoutGroup` правой колонки и панели «Участник» дочерние компоненты лежали на верхнем уровне узла вместо `props.components` — `renderLayoutGroup`/`renderPanel` читают детей только из `props`, возвращали `null`, из-за чего панели «Участник» и «Модерация» не отображались. Компоненты перенесены в `props.components`, панели возвращён `className: "w-full"`. Сканирование всех страниц на эту ошибку — чисто.

- **Карточка чека участника объединена с модерацией, `user-receipt-edit.json` удалена** (`config/pages/promo-instance/user-receipt.json`): страница приведена к единому виду с общей карточкой чека — таблица фото перенесена в панель «Информация о чеке» (панель «Фото чека» упразднена), в правой колонке под «Участником» — панель «Модерация» (статус, причина отклонения, кнопка «Применить»), кнопка «Модерировать» с страницы убрана (осталась «Удалить»); шорткат `saveReceipt` (PATCH) и `receiptStatuses` перенесены из edit-страницы. Отдельная страница `user-receipt-edit.json` (`/ops/users/[user_id]/receipts/[receipt_id]/edit`) удалена вместе с рефом в `config/crm-config.json` (страниц 34 → 33); API-маршруты PATCH/DELETE `ops/receipts/[receipt_id]` сохранены — их использует сама карточка.

- **Сокращение UUID в ячейках таблиц** (`src/utils/uuid.ts`, `src/engine/components/DataTableComponent.tsx`, 7 адаптеров): колонки таблиц можно помечать `"dataType": "uuid"` — значение ячейки выводится в краткой форме `01a0...afef` (4 первых + 4 последних символа; не-UUID значения проходят как есть). Маркер расставлен на 11 колонок: общие списки актов (`acts` — ID/Участник/Приз), призов (`prizes` — ID), чеков (`receipts` — ID/Участник), списки участника (`user-acts` — ID/Приз, `user-prizes` — ID, `user-products` — ID, `user-receipts` — ID). Текстовые компоненты и карточки-детейлы не затронуты — там UUID по-прежнему полные.

- **Клиентское форматирование дат в Text-компонентах (шаг 2, продолжение)** (`src/engine/components/TextComponent.tsx`, новый общий `src/engine/components/DateCell.tsx`, 10 страниц): компонент `Text` поддержал `props.dataType: "date"` — значение (биндинг на сырой ISO) форматируется в поясе зрителя после гидратации, той же SSR-безопасной схемой, что и таблицы; опциональный `props.dateFormat` (токены DD/MM/YYYY/HH/mm/ss, по умолчанию `DD.MM.YYYY HH:mm`). `DateCell` вынесен в общий модуль и переиспользуется DataTable и Text. Маркер расставлен на 16 дата-полей карточек/попапов: акты (`act.json`, `act-edit.json`, `user-act.json`, `user-act-edit.json` — «Создан»/«Обновлён»), попапы сообщений (`messages.json`, `user-messages.json` — «Дата»), чеки (`receipt.json`, `user-receipt.json`, `user-receipt-edit.json` — «Дата чека»/«Зарегистрирован»), документы (`user-docs.json` — «Дата выдачи», паттерн `DD.MM.YYYY`), карточка участника (`user.json` — «Дата регистрации»).

- **Форматирование дат в detail-таблицах** (`DataTableComponent.tsx`, `user.json`): для таблиц вида «параметр — значение» (строки `{label, value}`) дата-элемент строки можно пометить `"dataType": "date"` — тело ячейки форматируется через `DateCell` в поясе зрителя (паттерн — `dateFormat` колонки или строки). Признак учитывается, когда колонка сама не помечена `dataType`.

- **Клиентское форматирование дат в таблицах (шаг 2, применено ко всем таблицам)** (`src/utils/dateFormat.ts`, `src/engine/components/DataTableComponent.tsx`, 18 адаптеров): колонки с документированным маркером PrimeReact `dataType: "date"` рендерятся через `body`-функцию с `DateCell` — дата форматируется **в часовом поясе зрителя** после гидратации (SSR-безопасно, без hydration-mismatch). Паттерн задаётся в конфиге колонки (`dateFormat`), по умолчанию `DD.MM.YYYY HH:mm`. Маркер расставлен: в адаптерах с явными колонками (`users.get`, `user-acts`, `user-messages`, `acts`, `receipts`, `user-receipts`, `messages`) и автоматическим маппингом `type: datetime/date` → `dataType: date` в адаптерах формата columns/rows (11 шт.) и в колонках результата статистики (`stats.report.get.response.js`, из `fields[].type`). Адаптеры даты по-прежнему не форматируют — только передают сырой ISO и метаданные колонок. Эксперименты с `type: "datetime"` (неизвестный проп, игнорируется) и «голым» `dataType` (влияет только на режимы фильтра) — без эффекта, см. предыдущие коммиты.

- **Шаг 1 переноса форматирования дат на клиент: адаптеры больше не форматируют даты** (24 файла в `config/adapters/`): удалены вычисления полей `*_formatted` и вызовы `convertDateValue`/`convertDateColumns`; функции удалены из `_shared.js` (файл оставлен пустым). Даты во всех данных теперь сырые ISO (`created_at`, `registered_at`, `date`, `passport_issue_date` — последний нужен и для PATCH). Колонки таблиц, ссылавшиеся на `*_formatted`, переведены на сырые поля (`created_at`/`registered_at`) — в таблицах временно отображается ISO. Bindings страниц на `*_formatted` (попап сообщений, карточки чеков/документов) временно пусты — закроется на шаге 2 (клиентское форматирование: `type: "datetime"` у колонок DataTable, `format` у Text).
- **Ссылки страниц переведены с `*_formatted` на сырые поля** (7 страниц, 16 bindings/state-ключей): `messagesDialog.created_at` (попапы сообщений), `receiptData.registered_at`/`receiptData.date` (карточки чека), `docsData.passport_issue_date` (документы участника) — ссылки снова непустые; даты отображаются в сыром ISO до внедрения клиентского форматирования (шаг 2).
...
- **Автоколонки таблицы из данных** (`src/engine/components/DataTableComponent.tsx`): если у DataTable не задан конфиг `columns`, но `value` содержит строки — колонки строятся автоматически из ключей первой строки (header = имя поля, служебные `_*` исключаются, ISO-значения получают `dataType: date` с клиентским форматированием). Случай использования: результат SQL-отчёта статистики при пустом `fields` («все доступные поля») теперь выводится по всем столбцам ответа, хотя `reportData._columns` пуст.

- **`remote-docker-logs.sh` — просмотр логов контейнера** (`docker logs -f`): скрипт в корне репозитория (переименован из `logs.sh`); автоматически разрешает имя контейнера по списку кандидатов — `crm-frontend` (compose задаёт `container_name: crm-frontend`; `crm-frontend-crm-frontend` — это имя **образа**, совпадает со строкой) и `crm-frontend-crm-frontend`, при отсутствии обоих — единственный подходящий по маске `crm-frontend*`, иначе понятная ошибка со списком найденных контейнеров и подсказкой; предупреждение, если контейнер не запущен; дополнительные аргументы пробрасываются в `docker logs` (`--tail`, `--since` и т.д.), имя переопределяется через `CONTAINER=<имя>`. Задокументирован в README (раздел «Удалённый деплой»).
...
- **Декоративная стрелка в списке отчётов страницы «Статистика»** (`stats.json`) была активной кнопкой с командой `selectReport`: по её клику в контекст попадало событие кнопки, а не строка таблицы, поэтому выбор отчёта срабатывал некорректно. Стрелка стала неинтерактивной (`style.pointerEvents: "none"`, обработчик убран) — клик «сквозь» неё попадает в строку, которую обрабатывает `onRowClick` → `selectReport`.
- **Кнопка «К списку» на странице отчёта** (`stats-report.json`) вела на `/ops/stats` (рабочая страница статистики) вместо списка отчётов `/ops/stats/reports`.
...
- **Разделение страниц раздела «Статистика»** — страница работы с отчётами и страница управления отчётами разведены, добавлена навигация между ними.
  ...
  - `config/pages/promo-instance/stats-reports.json` (переименована из `stats.json`, маршрут `/ops/stats/reports`, заголовок «Отчёты статистики») — страница создания, настройки и обкатки отчётов; состав прежний (поиск, фильтр активности, «Создать отчёт», таблица со стрелками).
  - `config/menus/stats.json` (новая модель меню, по образцу `menus/user.json`) — пункты «Статистика» (`/ops/stats`) и «Управление отчетами» (`/ops/stats/reports`); обе страницы получили `menuSection` с Menubar (`activeItemId` — текущая страница); на `stats-report.json` верхнее меню не добавляется. Основное меню navbar без изменений («Статистика» → `/ops/stats`).
  - `config/crm-config.json` — реф переименованной страницы обновлён, новая страница добавлена (34 страницы); `stats-report.json` — крошка «Отчёты статистики» (`/ops/stats/reports`), возврат после удаления — на список отчётов.

- **Раздел «Статистика» промо-инстанса** — работа со статистическими отчётами (`docs/platform updates/Дополнение. Система статистики.md`).
  - `config/pages/promo-instance/stats.json` — страница `/ops/stats`: список отчётов (lazy-таблица: Название, Статус — Tag «Активен»/«Не активен»), поиск по названию (InputTextWithThrottle) и фильтр активности (Dropdown) по образцу страницы участников; кнопка «Создать отчёт» отправляет POST с телом-заглушкой `{title: "Новый отчёт", sql: "", fields: [], is_active: false}` и обновляет список (refresh); в строке кнопки «Запустить»/«Редактировать» и переход по клику — на страницу отчёта (без автозапуска).
  - `config/pages/promo-instance/stats-report.json` — страница `/ops/stats/[report_id]`: название (InputText), флаг активности (Checkbox), SQL (InputTextarea), поля выдачи (JSON-textarea → `fieldsText`); таблица результата (lazy, rows 10/20/50/100, колонки строятся адаптером из `fields[]` — `_columns`); кнопки «Запустить» (POST execute, результат в таблицу), «Выгрузить XLSX» (`downloadFile`, POST на file-маршрут export с `?format=xlsx`), «Сохранить» (PATCH; при невалидном JSON полей — error-тост), «Удалить» (confirm → DELETE → назад к списку), «К списку»; крошки «Статистика / {title}».
  - `config/system/api-routes.json` — маршруты instance-канала: `ops/stats/reports` GET/POST, `ops/stats/reports/[report_id]` GET/PATCH/DELETE, `ops/stats/reports/[report_id]/execute` POST (`query: true` — page/limit в query, §5.6 ТЗ), `ops/stats/reports/[report_id]/export` POST (`type: "file"` — бинарный стрим xlsx).
  - Адаптеры (зарегистрированы в `config/system/adapters.json`): `stats.reports.get.request` (first/rows→page/limit + search + is_active), `stats.reports.get.response` (unwrap `status/data`, метки активности), `stats.report.get.response` (unwrap + `fieldsText` + `_columns` из `fields[]`), `stats.report.patch.request` (тело из state, `JSON.parse(fieldsText)` с ошибкой при невалидном JSON), `stats.execute.request` (first/rows→page/limit, limit ≤ 100), `stats.execute.response` (`items/pagination` → value/totalRecords/first/rows).
  - `config/menus/navbar.json` — пункт «Статистика» (`pi pi-chart-bar`) в разделе «Промо-инстанс»; `config/crm-config.json` — страницы добавлены в `pages`.
...
- **Страницы статистики приведены к общим паттернам** (`stats.json`, `stats-report.json`):
  ...
  - из data шортката `runReport` (кнопка «Запустить») убрано `"event": "{$event}"` — для onClick-кнопки `{$event}` это React SyntheticEvent с DOM-узлом, его JSON.stringify падает с «Converting circular structure to JSON» (пагинация вынесена в отдельный шорткат, см. ниже);
  ...
  - кнопка «Создать отчёт» перенесена из шапки на панель фильтров (после селектора «Статус»);
  - обновление списка после создания отчёта: команда `refresh` заменена на повторный `sendRequest` списка + `mergeUrlParams` — `refresh` (default mode) вызывает `router.refresh()`, но AppEngine применяет новые SSR-данные dataFeed только при смене страницы (`prevPageIdRef !== newPageId`), поэтому на той же странице таблица не обновлялась; повторный запрос выполняется без поля `event` в data (вызов `requestReports` из onSuccess кнопки подставил бы `{$event}` = click-SyntheticEvent → circular JSON при сериализации, а адаптеру событие нужно только для пагинации onPage);
  - страница отчёта `stats-report.json` разбита на секции: `formMetaSection` — панель «Параметры отчёта» с названием (растягивается) и переключателем активности в одну строку (`LayoutGroup` flex); `formSqlSection` — две панели **в ряд** (штатная grid-раскладка секции: `col-12 md:col-6` + `col-12 md:col-6`, на узких экранах — стопкой): «SQL-запрос» и «Поля выдачи (JSON…)» (лейблы групп стали заголовками панелей, обёртки LabelledGroup убраны); `resultSection` и `buttonsSection` — без изменений. Всем кастомным секциям возвращены вертикальные отступы (`mb-5`): секции без `className` получают дефолтный `flex mb-5` (SectionRenderer), а заданный `className` его замещает — поэтому после добавления `w-full` отступы исчезали.
  - пагинация таблицы результата восстановлена: событие `onPage` таблицы вызывает новый шорткат `runReportPage` — его data содержит `"event": "{$event}"` (событие пагинатора — плоский сериализуемый объект, адаптер `stats.execute.request` берёт из него first/rows в приоритете), кнопка «Запустить» осталась на `runReport` (data из state, без события — иначе circular JSON); после успешного сохранения отчёта вместо неэффективного `refresh` выполняется повторный `GET /api/ops/stats/reports/{id}` → `state.reportData` — роутер применяет `stats.report.get.response`, и свежие `_columns` (построенные из сохранённых `fields[]`) приходят в state без перезагрузки страницы: колонки таблицы результата сразу соответствуют сохранённым полям;
  - панель «Параметры» на странице отчёта (новая секция `formParamsSection` между мета-секцией и SQL): дата-время-пикеры Calendar «Начало»/«Конец» (`showTime`, 24ч, иконка) → `state.params.startDate/endDate`; оба шортката запуска (`runReport`/`runReportPage`) передают с запросом `startDate`/`endDate`; request-адаптер `stats.execute.request` упаковывает их в параметр `replacements: { startDate, endDate }` (без подстановки на клиенте — замену макросов `{{startDate}}`/`{{endDate}}` в SQL выполняет сервер API перед запуском); сохранены правки интерфейса (чекбокс «Отчёт активен», `mb-0`).

### Fixed

- **В тостах об ошибках API не выводилась детализация `details`** — при ошибке вида `{"error":{"code","message","details":[{"field","issue"}]}}` пользователь видел только `message` (или общий текст из конфига onError), без указания конкретных полей и причин. Данные `details` при этом уже доходили до клиента (`parseApiError` парсит их, SSR и клиентские пути сохраняют ApiError-объект в `state.dataFeedErrors` и `onError`-контексте) — терялись они только при формировании текста тоста. Добавлен универсальный хелпер `formatApiError(error, baseMessage?)` (`src/utils/parseApiError.ts`: message + построчно `• field: issue`, лимит 10 строк с «…и ещё N», защита от дублей, принимает ApiError-объект, строку и null) и подключён в двух местах: `useDataFeedErrors` (тосты dataFeed-ошибок) и `executeShowToast` (`CommandExecutor` — если тост показывается в onError-контексте sendRequest/downloadFile, где ошибка доступна в extraSources `error` / `event.error`; важно: `createExtraSources` заворачивает контекст в `event`). Все существующие onError-тосты конфигов обогащаются автоматически, без правки конфигураций.

- **Выгрузка отчёта в XLSX не передавала даты подстановки** — API отвечал «данные подстановки не переданы». Две причины: (1) у file-маршрута `ops/stats/reports/[report_id]/export` не было request-адаптера — POST-тело уходило на внешний API «как есть» (`{startDate, endDate}` вместо ожидаемого `replacements`); (2) на странице отчёта `stats-report.json` кнопка «Выгрузить XLSX» вообще не отправляла `data`. Добавлен request-адаптер `stats.export.request` (тело `{format: "xlsx", replacements: {startDate, endDate}}` — пустые даты не включаются, page/limit для xlsx не отправляются: по §5.6 выгрузка идёт «файлом целиком»; format теперь и в теле, и в query — на whichever источник читает API) + кнопка страницы отчёта дополнена `data` с датами из `state.params`.

- **Страница «Статистика» (`stats.json`): вложенные блоки в `components` рендерились как «Не реализовано»** — панели «Параметры» и «Выполнение» были объявлены как `"type": "block"` внутри `components` блока, а блоки валидны только на уровне секции (`blocks`); ComponentRenderer не находил `componentType` и показывал заглушку. Перестроено на компоненты `componentType: "Panel"` (дети — в `props.components`, как поддерживает `renderPanel`): «Параметры» (startDate/endDate в столбик) и «Выполнение» (LayoutGroup с кнопками «Запустить»/«Выгрузить XLSX»).
...
- **Маршруты management-канала для карточки чека** — в `config/system/api-routes.json` добавлены `mgmt/receipts/[receipt_id]` (GET/PATCH/DELETE), `mgmt/receipts/[receipt_id]/products` (GET) и `mgmt/receipts/[receipt_id]/check` (POST — автомодерация); URL всех клиентских запросов и dataFeed страниц раздела переведены с несуществующего `/api/receipts/...` на `/api/mgmt/receipts/...`, из dataFeed убран нестандартный префикс `{$config.baseURL}`; исправлены относительные `$ref` к `list-request-format.json` в `receipt.json`/`receipt-edit.json` (`../system/...` → `../../system/...`).
...
- **Переключение кампаний не влияло на данные страниц** — SSR-загрузка данных шла всегда в первую кампанию из `camps.json`. Причина: серверный dataFeed-сервис (`src/core/DataFeedServerService.ts`) при исполнении dataFeed'ов делает внутренний fetch на собственный API-роутер, но не пересылал cookies пользователя — роутер не видел `camp_id` и по фолбэку брал первую кампанию списка. Теперь SSR-запросы пересылают заголовок `Cookie` (все cookies из `serverSources.cookies`) — выбирается кампания пользователя; попутно в роутер стал попадать и auth-cookie (`access_token`), что чинит чтение токена management-канала в SSR. Клиентские запросы (lazy-таблицы, sendRequest) и так несли cookie браузером — на них баг не влиял.

- **«Ошибка загрузки данных» после правки пересылки cookies в SSR** — серверный dataFeed-сервис отправлял значения cookies в заголовок `Cookie` внутреннего запроса «как есть», а cookie `user_data` содержит JSON с кириллическим именем пользователя. `fetch()` (undici) отвергает такие значения с ошибкой `Cannot convert argument to a ByteString` — все SSR-фиды падали, и клиент показывал тост «Ошибка загрузки данных» (`summary` из `useDataFeedErrors`). Значения cookies в SSR-запросе теперь percent-кодируются (`encodeURIComponent`) — заголовок всегда ASCII, а роутер при разборе получает исходные значения (JWT и `camp_id` кодирование не меняет).

- **Роутер больше не пересылает входной заголовок `Cookie` во внешний CRM API** — он не нужен: management-канал авторизуется через `Authorization`, instance-канал — через HMAC-подпись. Пересылка лишь уносила на хост кампании сессионные cookies панели (`access_token`, `refresh_token`, `user_data`) и создавала тот же риск ByteString-ошибки при не-ASCII значениях.

- **Правки списков и страниц чеков/актов/документов**:
  - список актов участника (`user-acts.response.js`) — убран столбец «Обновлён»;
  - списки чеков (`receipts.response.js`, `user-receipts.response.js`) — добавлены столбцы «Статус ФНС» (`fns_status`, фолбэк «—») и «Акционных» (`promo_products`) в список чеков участника (в общем списке «Продуктов»/«Акционных» уже были);
  - карточка чека участника (`user-receipt.json`) — панель «Фото чека» переведена с превью-изображений на таблицу с кнопками «Скачать» (как в документах участника; фото уже идут через файловый прокси);
  - список документов (`user-docs.json`) — убран столбец «ID файла»;
  - страница модерации чека участника (`user-receipt-edit.json`) — добавлена панель «Продукция в чеке» с таблицей продукции (dataFeed на `/api/ops/receipts/{id}/products`, как на карточке чека).
...
- **Пустые данные на страницах документов участника** — dataFeed страниц `user-docs`/`user-docs-edit` запрашивал `ops/users/[user_id]`, чей response-адаптер `user.get.response` (карточка участника) применялся API-роутером раньше feed-адаптера и отбрасывал `extra`: все поля (паспорт, ИНН) приходили пустыми, а документы из `data.extra.documents` терялись. Добавлен выделенный маршрут GET `ops/users/[user_id]/docs` (тот же внешний эндпоинт 3.2.2 `GET /api/v1/crm/users/{id}`) со своим response-адаптером.
  ...
  - `config/pages/user-docs-edit.json` — dataFeed переключён на тот же маршрут (раньше форма открывалась пустой, а сохранение затирало расширенные данные пустыми строками).
  - `config/system/adapters.json` — удалены мёртвые регистрации `user-docs.response` и `user-docs.get.request`, файлы адаптеров удалены.

- **Универсальный файловый прокси `ops/files/**`** — скачивание любых файлов, пути к которым приходят в ответах API без префикса версии (`/users/profile/extra/document/{id}` — документы участника, `/receipt/{id}/photo/{файл}` — фото чека, `/acts/{id}` — акт). `matchRoutePattern` в `src/app/api/[...route]/route.ts` поддержал хвостовой wildcard `**`: остаток пути захватывается в `routeParams.path`; маршрут GET `ops/files/**` (`type: "file"`) проксирует бинарный ответ `{crm_api_url}/api/v1/crm/{путь}` с авторизацией и HMAC-подписью instance-канала. Используется кнопкой «Скачать» на странице документов; фото чеков и бланк акта переводятся на прокси отдельной задачей.
...
- **Доработки карточек-детейлов и списков сообщений**:
  ...
  - Попап сообщения — форма на компонентах движка: в заголовке — код обращения; в верхней строке слева блок отправителя (`Avatar` с инициалами и фолбэк-иконкой + имя/e-mail), справа — дата и статус (`Tag`, severity из `status_severity`: delivered → success, failed → danger); разделитель `Divider`; «Тема обращения» и «Сообщение» — `LabelledGroup` (полный текст с переносами строк); раскладка — `LayoutGroup`. Кнопки: «Написать письмо» (команда `navigate` с готовым `mailto:{email}?subject=…` из `mailto_link` адаптера, активна при наличии e-mail) и «Закрыть», справа. Строки таблиц сообщений активны, как в остальных списках (`selectionMode: "single"`).
  - `src/core/CommandExecutor.ts` — `executeNavigate`: ссылки `mailto:`/`tel:` открываются напрямую через `window.location.href` (роутер Next.js их не обрабатывает).
...
- **Общие страницы актов (операционный канал)** — порт legacy-страниц `acts`/`act`/`act-add`/`act-edit` на CRM API (список — 1.7 `GET /api/v1/crm/acts`, детали — 1.8 `GET /api/v1/crm/acts/[id]`, создание — 1.9 `POST /api/v1/crm/acts`, обновление — 1.10 `PATCH /api/v1/crm/acts/[id]`, удаление — 1.11 `DELETE /api/v1/crm/acts/[id]`) по образцу общих страниц чеков и страниц актов участника: без пользовательского меню, с хлебными крошками.
  ...
  - `config/pages/act.json` — карточка акта `/ops/acts/[act_id]`: панель «Информация об акте» (участник-ссылка, статус `Tag`, даты, «Бланк документа» — заглушка скачивания до появления файлового прокси) и панель «Приз» (ID приза, переход к карточке приза `/ops/prizes/[prize_id]`); кнопки Редактировать/Удалить (confirm → DELETE → toast → список).
  - `config/pages/act-edit.json` — редактирование акта `/ops/acts/[act_id]/edit`: readonly-информация об акте, редактирование только статуса (API PATCH принимает только `status`), кнопки Сохранить/Применить/Отменить изменения.
  - `config/pages/act-add.json` — добавление акта `/ops/acts/add`: выбор участника (список `/api/ops/users`), зависимый выбор приза (подгрузка призов выбранного участника по `onChange` с очисткой выбора при смене участника), статус; `POST /api/ops/acts` с `user_id`/`prize_id`/`status`.
  - `config/adapters/act.get.response.js` — добавлено `user_id_label` (`user_id` или «—») для панели «Участник» общей карточки; остальной формат без изменений (используется и страницами акта участника).
  - `config/system/api-routes.json` — GET `ops/acts` переключён с `ops.pagination.response` на связку `users.get.request` + `acts.response` (пагинация `page`/`limit`, дефолт 25); POST/PATCH/DELETE `ops/acts/[act_id]` и GET деталей были зарегистрированы ранее.
  - `config/system/adapters.json`, `config/crm-config.json` — регистрация адаптера `acts.response` и страниц `act.json`, `act-add.json`, `act-edit.json`.

- **Общие страницы призов — пул (операционный канал, только просмотр)** — порт legacy-страниц `prizes`/`prize` на CRM API (список — 7.1 `GET /api/v1/crm/prizes`, детали — 7.3 `GET /api/v1/crm/prizes/[id]`) по образцу общих страниц чеков: без пользовательского меню, с хлебными крошками, без кнопок редактирования/модерации (пул призов — только просмотр).
  - `config/pages/prizes.json` — список призов `/ops/prizes` переписан со старого формата на lazy `DataTable` (колонки: ID, Название, Стоимость, Статус, Активен, Владелец; `rowsPerPageOptions` [5,10,25,50]); клик по строке/стрелке → карточка приза; шорткат `requestPrizes` (`page`/`limit` + `mergeUrlParams`).
  - `config/adapters/prizes.response.js` (новый) — `{items, pagination}` → `{value, columns, totalRecords, first, rows}`: статусы через словарь (лейбл + severity), `is_active` → «Да»/«Нет», `user_id` → «Владелец» с фолбэком «—» (у призов пула владелец может отсутствовать), фолбэк размера страницы 25.
  - `config/pages/prize.json` — карточка приза `/ops/prizes/[prize_id]`: панель «Информация о призе» (название, стоимость в баллах, статус `Tag`, «Требуется акт», «Активен», «Недостающие данные») и панель «Владелец» (ID участника или «—»); без действий — только просмотр.
  - `config/adapters/prize.get.response.js` — добавлено `user_id_label` (`user_id` или «—») для карточки пула; остальной формат без изменений (используется и страницей приза участника).
  - `config/system/api-routes.json` — GET `ops/prizes` переключён с `ops.pagination.response` на связку `users.get.request` + `prizes.response` (пагинация `page`/`limit`, дефолт 25).
  - `config/system/adapters.json`, `config/crm-config.json` — регистрация адаптера `prizes.response` и страницы `prize.json`.

- **Общие страницы чеков (операционный канал, без привязки к участнику)** — порт legacy-страниц `receipts`/`receipt`/`receipt-edit` на CRM API (список — `GET /api/v1/crm/receipts`, детали — 2.12 `GET /api/v1/crm/receipts/[id]`, продукция — 2.13 `GET .../[id]/products`, модерация — 2.14 `PATCH /api/v1/crm/receipts/[id]`, удаление — 2.15 `DELETE /api/v1/crm/receipts/[id]`) по образцу страниц чеков участника: без пользовательского меню, с хлебными крошками.
  - `config/pages/receipts.json` — список чеков `/ops/receipts` переписан со старого формата на lazy `DataTable` (колонки: ID, Участник, Статус, Продуктов, Акционных, Зарегистрирован; `rowsPerPageOptions` [5,10,25,50]); клик по строке/стрелке → карточка чека, кнопка-иконка «пользователь» → карточка участника; шорткат `requestReceipts` (`page`/`limit` + `mergeUrlParams`).
  - `config/adapters/receipts.response.js` (новый) — `{items, pagination}` → `{value, columns, totalRecords, first, rows}`: статусы через словарь (лейбл + severity для Tag), счётчики продукции/акционных с фолбэком «—», форматирование даты регистрации, `user_id` сохраняется для перехода к участнику, фолбэк размера страницы 25.
  - `config/pages/receipt.json` — карточка чека `/ops/receipts/[receipt_id]`: панель «Информация о чеке» (ID, статус `Tag`, сумма из копеек в рубли, дата покупки, регистрация, ФН/ФД/ФП), панель «Фото чека» (`DataTable` + `Image` с превью из `photos[].url`), панель «Продукция в чеке», панель «Участник» (переход к карточке участника по `user_id`), кнопки «Модерировать»/«Удалить» (confirm → DELETE → тост → возврат к списку).
  - `config/pages/receipt-edit.json` — модерация чека `/ops/receipts/[receipt_id]/edit`: readonly-информация о чеке, `Dropdown` статуса (CHECKING/ACCEPTED/REFUSED/SUSPENDED/NEED_PHOTO), `InputTextarea` причины отклонения, шорткат `saveReceipt` (PATCH `status` + `decline_reason`), кнопки Сохранить/Применить/Отменить.
  - `config/system/api-routes.json` — GET `ops/receipts` переключён с `ops.pagination.response` на связку `users.get.request` + `receipts.response` (пагинация `page`/`limit`, дефолт 25); эндпоинты 2.12–2.15 уже зарегистрированы ранее.
  - `config/system/adapters.json`, `config/crm-config.json` — регистрация адаптера `receipts.response` и страниц `receipt.json`/`receipt-edit.json`.
