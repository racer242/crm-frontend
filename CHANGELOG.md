...
- **Страница редактирования розыгрыша приведена к образцу user-edit** (`config/pages/promo-instance/raffle-edit.json`, `config/adapters/raffle.patch.request.js`): хлебные крошки — Розыгрыши → `{name}` (ссылка на карточку) → «Редактирование»; панель «Информация о розыгрыше» убрана — осталась только форма из двух панелей («Параметры розыгрыша»: Название, Тип (InputText), Типы источников шансов (MultiSelect по `source_types`, 6 опций), Дата публикации (Calendar showTime), «Срок сбора шансов»: два Calendar showTime); кнопки «Опубликовать»/«Аннулировать» перенесены на карточку (шорткаты `publishRaffle`/`annulRaffle` удалены, остались «Сохранить»/«Применить»/«Отменить изменения»); PATCH-адаптер отправляет `name`, `type` (пустая строка → `null` — тип можно очистить), `source_types` (массив), `publish_at` и плоские `collection_from`/`collection_to` — по форме актуального GET (вложенный `collection_period` больше не собирается).
