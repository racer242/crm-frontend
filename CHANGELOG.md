...
- **Страницы статистики приведены к общим паттернам** (`stats.json`, `stats-report.json`):
  ...
  - страница отчёта `stats-report.json` разбита на секции (все секции и блоки — на всю ширину, `w-full`): `formMetaSection` — панель «Параметры отчёта» с названием (растягивается) и переключателем активности в одну строку (`LayoutGroup` flex); `formSqlSection` — две панели друг под другом: «SQL-запрос» и «Поля выдачи (JSON…)» (лейблы групп стали заголовками панелей, обёртки LabelledGroup убраны); `resultSection` и `buttonsSection` — без изменений.
