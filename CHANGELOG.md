...
- **Счётчики на карточке участника** (`config/pages/promo-instance/user.json`, `config/adapters/users.get.request.js`): StatCard «Чеки»/«Продукты»/«Сообщения»/«Призы» заполнены реальными числами — 4 добавленных SSR-dataFeed'а с `rows=1` берут счётчик из `pagination.total_items` списков (response-адаптеры кладут его в `totalRecords`); биндинги переключены с пустых legacy-узлов (`productData`/`feedbackData`/`achevementsData` — удалены) на `state.receiptsStats/productsStats/messagesStats/prizesStats.totalRecords`. Счётчик чеков считается по общему списку чеков 2.11 с фильтрами `user_id` + `status=ACCEPTED` (подзаголовок «проверены»), т.к. список чеков участника 2.16 фильтра статуса не имеет; для этого request-адаптер `users.get.request` теперь пробрасывает `user_id` (остальные маршруты не передают его — поведение не изменилось). «Баллы» — как раньше, из профиля.
