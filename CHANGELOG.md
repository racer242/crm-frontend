...
- **Клиентское форматирование дат в таблицах (шаг 2, первый шаг)** (`src/utils/dateFormat.ts`, `src/engine/components/DataTableComponent.tsx`): колонки с документированным маркером PrimeReact `dataType: "date"` рендерятся через `body`-функцию с компонентом `DateCell` — дата форматируется **в часовом поясе зрителя** после гидратации (SSR-безопасно: сервер и первый клиентский рендер показывают сырое значение, без hydration-mismatch). Паттерн задаётся в конфиге колонки (`dateFormat`, токены DD/MM/YYYY/HH/mm/ss), по умолчанию `DD.MM.YYYY HH:mm`. Адаптеры даты не форматируют — маркер уже стоит на колонке «Дата» в `messages.response.js`; остальные таблицы подключаются добавлением `dataType: "date"` в колонки их адаптеров. Эксперименты с `type: "datetime"` (неизвестный проп, игнорируется) и «голым» `dataType` (влияет только на режимы фильтра) — без эффекта, см. предыдущие коммиты.
