...
main() {
  ...
  # Remove previously deployed config to avoid stale files accumulating
  # (pages/adapters renamed or deleted locally would otherwise linger on remote)
  ssh -p "$REMOTE_PORT" "$REMOTE_USER@$REMOTE_HOST" "rm -rf $REMOTE_DIR/config $REMOTE_DIR/messages"
  ...
  # --force-recreate is required: docker compose up -d alone does NOT recreate
  # the container when only mounted files (config/, messages/) have changed,
  # so the app would keep serving the previous configuration.
  ssh -p "$REMOTE_PORT" "$REMOTE_USER@$REMOTE_HOST" \
    "cd $REMOTE_DIR && \
     sudo HOST_PORT=$HOST_PORT docker compose up -d --force-recreate && \
     echo '' && \
