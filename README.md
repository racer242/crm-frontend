...
| **Источник данных (Backend)** | Поддержка двух каналов: управляющий бэкенд (JWT) и промо-инстансы (HMAC-SHA256 подпись) |
...
- **crypto** — HMAC-SHA256 подпись запросов к промо-инстансам
...
│   ├── pages/                   # Конфигурации страниц (promo-instance/ — промо-инстанс, crm-management/ — CRM-управление)
...
Рабочие страницы CRM-конфигурации описываются в `config/crm-config.json` (`config/pages/promo-instance/`): участники, документы, акты, призы, чеки, покупки, сообщения, журнал событий и история баллов участника (`/ops/users/[user_id]/...`, журнал — `/ops/users/[user_id]/log`, баллы — `/ops/users/[user_id]/points/history` — «История начисления и списания баллов» с фильтром по типу операции; карточка чека участника, как и общая карточка чека, объединяет просмотр и модерацию — отдельная страница редактирования удалена; карточка участника показывает счётчики: чеков всего (список чеков участника) и принятых (список чеков с фильтрами user_id+status=ACCEPTED), продуктов, сообщений и призов — числа берутся из пагинации списков (limit=1, total_items), у счётчиков — кнопки-стрелки (StatCard `actionIcon`) с переходом на соответствующие списки участника (для принятых чеков — с фильтром status=ACCEPTED из URL), а также общие страницы операционного канала — чеки (`/ops/receipts` — поиск и фильтры по статусу модерации и статусу ФНС, синхронизация с URL; карточка чека объединяет просмотр и модерацию — статус, причина отказа, сохранение/удаление), пул призов (`/ops/prizes`), акты (`/ops/acts`, добавление и редактирование), розыгрыши (`/ops/raffles` — список, карточка со всеми полями API, панелью «Управление» (публикация, начало/завершение проведения, расчёт шансов, черновик, аннулирование, протокол), кнопкой «Редактировать» в низу панели «Информация о розыгрыше» под разделителем (активна по флагу `is_editable`) и панелью «Шансы розыгрыша» — lazy-таблица шансов выборки (§8.9) с колонками из фактических полей ответа; редактирование — форма из панелей «Параметры розыгрыша» (название, тип, типы источников шансов — MultiSelect) и «Даты розыгрыша» (сбор и публикация) по образцу карточки участника, источники шансов — MultiSelect) и сообщения обратной связи (`/ops/messages` — поиск + фильтр по теме из справочника `feedback_topics` (`ops/dictionaries/[name]` → `/api/v1/crm/dictionaries/[name]`); список с колонками дата/имя/e-mail отправителя/тема/сокращённое сообщение; по клику на строку открывается модальное окно со всеми данными сообщения: тема, код обращения, отправитель, статус доставки, дата, полный текст — с кнопками «Написать письмо» (mailto) и «Закрыть»). Скачивание и просмотр файлов (документы участника, фото чеков, бланк акта) идёт через универсальный файловый прокси `ops/files/**`: `/api/ops/files/<путь из ответа API>` → `{crm_api_url}/api/v1/crm/<путь>`; фото чеков и бланк акта переписываются на прокси в адаптерах (`receipt.get.response.js`, `act.get.response.js`), сырые пути сохраняются в `url_raw`/`file_url_raw`. Маршруты страниц и правила матчинга — [docs/config-reference.md](docs/config-reference.md), реестр API-роутов — `config/system/api-routes.json`. Страницы канала CRM-управления описаны в `config/pages/crm-management/` — модерация чеков (`/mgmt/receipts`, маршрут `mgmt/receipts` management-канала: `{base_api_url}/receipts`, адаптеры `mgmt.receipts.get.*`; карточка чека `/mgmt/receipts/[receipt_id]` объединяет просмотр и модерацию — панель «Модерация» (торговая сеть, статус, комментарий, кнопки «Автомодерация»/«Применить»), участник подгружается SSR-цепочкой `GET /api/ops/users/{$state.receiptData.user_id}`, отдельная страница редактирования удалена). Раздел «Статистика» (`config/pages/promo-instance/stats.json`, `stats-reports.json`, `stats-report.json`; меню `config/menus/stats.json` с переключением «Статистика»/«Управление отчетами») — страница работы с отчётами (`/ops/stats`: слева поиск и таблица только активных отчётов, выбор строки заполняет правую панель — название отчёта, параметры периода startDate/endDate, кнопки «Запустить»/«Выгрузить XLSX»; результат — lazy-таблица по колонкам отчёта), страница управления отчётами (`/ops/stats/reports`: таблица с поиском по названию и фильтром активности, кнопка «Создать отчёт» создаёт отчёт-заглушку и обновляет список) и страница отчёта (`/ops/stats/[report_id]`: название, флаг активности, SQL, JSON-редактор полей выдачи, кнопки «Запустить» (результат — в lazy-таблице, постранично), «Выгрузить XLSX» (file-маршрут `ops/stats/reports/[report_id]/export`, POST + `?format=xlsx`), «Сохранить» (PATCH), «Удалить»; выполнение — POST `ops/stats/reports/[report_id]/execute` с флагом `query: true` — page/limit уходят в query).
...
Файлы `.legacy`, `.skip-read`, `config` исключены из docker build context (`.dockerignore`). **`messages/` в контексте остаётся**: Turbopack при сборке сканирует эту папку для динамического импорта локалей в `src/i18n/request.ts` — без неё сборка падает с `Module not found`. В runner-образ ни `config`, ни `messages` не попадают — монтируются с хоста.
