...
Рабочие страницы CRM-конфигурации описываются в `config/crm-config.json` (`config/pages/`): участники, документы, акты, призы, чеки, покупки и сообщения участника (`/ops/users/[user_id]/...`), а также общие страницы операционного канала — чеки (`/ops/receipts`), пул призов (`/ops/prizes`), акты (`/ops/acts`, добавление и редактирование) и сообщения обратной связи (`/ops/messages` — список с колонками дата/имя/e-mail отправителя/тема/сокращённое сообщение; полный текст открывается в модальном окне по клику на строку). Скачивание файлов (документы участника, фото чеков, сканы актов) идёт через универсальный файловый прокси `ops/files/**`: `/api/ops/files/<путь из ответа API>` → `{crm_api_url}/api/v1/crm/<путь>`. Маршруты страниц и правила матчинга — [docs/config-reference.md](docs/config-reference.md), реестр API-роутов — `config/system/api-routes.json`.
